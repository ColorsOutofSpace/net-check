...
- 连通性：`Ping`、默认路由检测、默认网关可达性、ARP 邻居表检测、全局 ICMP 探测。
- DNS：`DNS 解析`、DNS 服务器配置、DNS 服务器可用性、Hosts 文件检测、全局 DNS 探测。
- 路径分析：路由追踪。
- HTTP：`HTTP 可用性`。
- 网络接口：网卡链路状态、虚拟网卡与路由检测、网卡 IP 配置、DHCP 状态。
- 代理与环境：`IE 代理检测`、WinHTTP 代理检测、代理冲突检测、`LSP 目录检测`、网络环境变量检测。
